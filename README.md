...
*   Cleaned datasets files for all the languages(Xitsonga(dev_cleaned , test_cleaned , train_cleaned) , sesotho_cleaned ,setswana_cleaned).
...
1.  **Upload Cleaned Data:** Ensure the **cleaned** `.csv` files (generated by the preprocessing scripts, e.g., Xitsonga(dev_cleaned , test_cleaned , train_cleaned) , sesotho_cleaned ,setswana_cleaned) are uploaded to the Google Drive path specified in Cell A (e.g., `/content/drive/MyDrive/NLPS1/Cleaned/`).
